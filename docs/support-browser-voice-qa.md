...
- Before that attestation, only the exact bounded xAI `session.created`, `conversation.created`, and `ping` control envelopes are ignored. The pre-update `session.created` snapshot permits only xAI's observed content-free `turn_detection` defaults: `null` or exact `{ "type": "server_vad" }`; neither can grant readiness. A valid content-free `ping` is also ignored during greeting or an attested session; a late session/conversation creation event terminates instead of resetting state. No control can make the browser ready or alter the authoritative session, and malformed, unknown, or capability-bearing messages remain fail-closed.
