# Tavus webhook payload validation — QA contract

This document describes the focused inbound payload contract for alphaScreen's active `POST /webhook/tavus` callback. Sender authentication remains the first route middleware. The shared Tavus outbound HTTP client is not part of this contract.
...
| `conversation.utterance` | consumed role/speech fields must have safe types | existing lifecycle/perception ingestion |
| `conversation.perception_tool_call` | bounded tool name; arguments string/object if present | existing gated perception ingestion |
