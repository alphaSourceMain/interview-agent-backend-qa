...
function buildConversationalContext(candidateName, roleTitle, companyName, rubricQuestions = [], roleSpecificPrompt = '', maxInterviewMinutes = null) {
  ...
  lines.push(
    ...
    '- When runtime control state becomes closing_window, let any active candidate answer finish, then at the next natural turn boundary stop new substantive questions and move to the closing sequence.',
    '- In closing_window, invite one final candidate question when practical. If there is not enough time, use the final closing line instead.',
    '- When runtime control state becomes termination_window, do not start a question flow. Briefly use the final closing line and call end_interview without waiting for candidate acknowledgment.',
    ...
    '- Invite one final candidate question when practical, and do not repeat the invitation.',
    '- If the candidate asks an answerable live interview mechanics question at the end, answer it briefly in no more than 2 sentences, then close immediately with the final closing line and end_interview.',
    '- If the candidate asks no question, declines, or indicates they are finished, use the final closing line and end_interview immediately.',
    '- Do not require a candidate acknowledgment after answering the final question or giving the final closing line.',
    '- Do not sit silently after the final answer, a decline, or the final closing line. End immediately.',
  ...
  lines.push(
    ...
    '- In closing_window, finish the active turn and use the one-question closing sequence when practical. In termination_window, use the final closing line and end_interview without waiting for acknowledgment.',
