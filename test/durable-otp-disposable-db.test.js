...
test('SMS monitoring snapshot is aggregate, service-only, and capability-aware', { skip: !ENABLED }, () => {
  ...
  assert.equal(snapshot.capabilities.spend_monitoring, true);
  assert.equal(snapshot.capabilities.line_type_monitoring, true);
  assert.equal(snapshot.capabilities.inbound_control_monitoring, true);
  assert.equal(snapshot.capabilities.provider_breaker_monitoring, true);
