...
test('bounded states, scope binding, and provider identity uniqueness fail closed', { skip: !ENABLED }, () => {
  ...
  sql("set role service_role; insert into public.pronunciation_dictionary_syncs(dictionary_key,environment,provider,provider_dictionary_id) values('one','qa','tavus','provider-1');");
  assert.notEqual(sql("set role service_role; insert into public.pronunciation_dictionary_syncs(dictionary_key,environment,provider,provider_dictionary_id) values('two','qa','tavus','provider-1');", { allowFailure: true }).status, 0);
});
