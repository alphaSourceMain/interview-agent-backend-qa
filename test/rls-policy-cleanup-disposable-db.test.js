...
const MIGRATION_SUFFIX = '_manager_member_rls_policy_cleanup.sql';
...
test('private scope helper is locked down and supported by membership indexes', { skip: !ENABLED || !APPLY_MIGRATION }, () => {
  ...
  for (const table of TARGET_TABLES) {
    ...
    const plan = sql(`set enable_seqscan=off; explain (costs off) select id from public.${table} where client_id='${CLIENT_PARENT}'`).stdout;
